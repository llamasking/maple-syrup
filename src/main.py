...
async def command_generate_gpt(
    ...
    embed.set_footer(text=f"AI Model: {config['TEXT_GENERATION_MODEL']} -- Generation took {end_time-start_time:.2f}s")
...
async def command_converse(ctx: ApplicationContext, message: str):
    ...
    # Design embed
    embed = Embed(title="Text Generation Results", color=3743206)
    embed.set_footer(text=f"AI Model: {config['CONVERSATION_GENERATION_MODEL']} -- Generation took {end_time-start_time:.2f}s")
